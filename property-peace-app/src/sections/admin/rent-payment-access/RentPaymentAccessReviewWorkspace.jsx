...
export default function RentPaymentAccessReviewWorkspace() {
  ...
  return <Stack spacing={3}>
    ...
    {loading ? <Box sx={{ py: 8, textAlign: 'center' }}><CircularProgress /></Box> : publicId ? <Stack spacing={2}>
      ...
      <Paper variant="outlined" sx={{ p: 3 }}><Stack spacing={1.25}><Stack direction="row" spacing={1} alignItems="center"><Typography variant="h4">{organizationName}</Typography><Chip size="small" label={status || 'Unknown'} /></Stack><Typography variant="body2">Organization ID: {detail?.organizationId || detail?.OrganizationId || '—'}</Typography><Typography variant="body2">Requester: {detail?.requesterEmail || detail?.RequesterEmail || '—'} · Requested {date(detail?.requestedAt || detail?.RequestedAt)}</Typography><Typography variant="body2" color="text.secondary">Reason: {detail?.decisionReason || detail?.DecisionReason || '—'}</Typography><Typography variant="subtitle2" sx={{ mt: 1 }}>Audit timeline</Typography>{(detail?.auditTimeline || detail?.AuditTimeline || []).map((item, index) => <Typography key={item.id || index} variant="body2" color="text.secondary">{date(item.createdAt || item.CreatedAt)} · {item.status || item.Status}</Typography>)}<Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} sx={{ mt: 1 }}>{status !== 'Approved' && <Button variant="contained" onClick={() => openDecision('approve')}>Approve access</Button>}{status !== 'Rejected' && <Button color="warning" variant="outlined" onClick={() => openDecision('reject')}>Reject request</Button>}{status !== 'Suspended' && <Button color="error" variant="outlined" startIcon={<StopOutlined />} onClick={() => openDecision('suspend')}>Suspend access</Button>}</Stack>{(detail?.stripeAccountId || detail?.StripeAccountId) && <Button variant="text" onClick={() => navigate('/admin/stripe-payees')}>Open separate Stripe payee review</Button>}</Stack></Paper>
    </Stack> : <><Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" spacing={2}><TextField select size="small" label="Status" value={filter} onChange={(event) => setFilter(event.target.value)} sx={{ minWidth: 180 }}>{filters.map((item) => <MenuItem key={item} value={item}>{item}</MenuItem>)}</TextField><Button variant="outlined" startIcon={<ReloadOutlined />} onClick={load}>Refresh</Button></Stack><Stack spacing={1.5}>{requests.length ? requests.map((request) => <Paper key={request.publicId || request.PublicId} variant="outlined" sx={{ p: 2 }}><Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" spacing={1}><Box><Typography variant="h6">{request.organizationName || request.OrganizationName}</Typography><Typography variant="body2" color="text.secondary">{request.requesterEmail || request.RequesterEmail} · {date(request.requestedAt || request.RequestedAt)}</Typography></Box><Button onClick={() => navigate(`/admin/rent-payment-access/${request.publicId || request.PublicId}`)}>Review</Button></Stack></Paper>) : <Paper variant="outlined" sx={{ p: 4, textAlign: 'center' }}><Typography color="text.secondary">No requests match this filter.</Typography></Paper>}</Stack></>}
