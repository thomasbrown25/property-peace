...
const getFullName = (tenant) =>
  `${read(tenant, 'firstname', 'Firstname') || ''} ${read(tenant, 'lastname', 'Lastname') || ''}`.trim() || 'Unnamed tenant';
...
function PortalStatus({ tenant, tenantInvites }) {
  ...
  return (
    <Chip label="Email needed" size="small" variant="outlined" sx={{ color: 'text.secondary', borderColor: 'divider', fontWeight: 600 }} />
  );
}
  ...
  return (
    ...
    >
      <Box sx={{ display: { xs: 'none', md: 'block' } }} onClick={(event) => event.stopPropagation()}>
        <Tooltip
          title={!email ? 'Add an email address before inviting' : hasAccount(tenant) ? 'Portal account already active' : ''}
          ...
          <>
            ...
            <Stack direction="row" spacing={0.7} alignItems="center" sx={{ mt: 0.45 }}>
              ...
              <Typography
                sx={{
                  fontSize: '0.7rem',
                  color: isLeaseEndingSoon(tenant) ? 'warning.main' : 'text.secondary',
                  fontWeight: isLeaseEndingSoon(tenant) ? 650 : 400
                }}
              >
                {hasLease(tenant)
                  ? leaseEnd
      ...
      <Stack spacing={0.5} sx={{ mt: { xs: 1.25, md: 0 }, pl: { xs: 6.5, md: 0 }, minWidth: 0 }}>
        <Stack direction="row" spacing={0.7} alignItems="center" minWidth={0}>
          ...
          <Typography noWrap sx={{ fontSize: '0.76rem', color: email ? 'text.secondary' : 'text.disabled' }}>
            {email || 'No email on file'}
          </Typography>
        </Stack>
        <Stack direction="row" spacing={0.7} alignItems="center" minWidth={0}>
          <PhoneOutlined style={{ fontSize: 13, color: theme.palette.text.secondary, flexShrink: 0 }} />
          <Typography noWrap sx={{ fontSize: '0.76rem', color: phone ? 'text.secondary' : 'text.disabled' }}>
            {phone ? formatPhoneInput(phone) : 'No phone on file'}
          </Typography>
        </Stack>
      </Stack>
      ...
      <Box
        sx={{ display: 'flex', justifyContent: { xs: 'flex-end', md: 'center' }, mt: { xs: -4, md: 0 } }}
...
export default function TenantsContent({ embedded = false }) {
        ...
        .map(async (tenant) => {
          try {
            ...
            const hasValidInvite = invites.some(
              (invite) => !invite.isUsed && (!invite.expiresAt || new Date(invite.expiresAt) > new Date())
            );
  ...
  const filteredTenants = useMemo(() => {
    ...
    return list.sort((a, b) => {
      ...
      if (sort === 'leaseEnd')
        return (getLeaseEnd(a)?.getTime() || Number.MAX_SAFE_INTEGER) - (getLeaseEnd(b)?.getTime() || Number.MAX_SAFE_INTEGER);
      if (sort === 'newest')
        return (
          (parseDate(read(b, 'createdAt', 'CreatedAt'))?.getTime() || 0) - (parseDate(read(a, 'createdAt', 'CreatedAt'))?.getTime() || 0)
        );
  ...
  const hasFilters =
    search || recordStatus !== 'active' || propertyFilter !== 'all' || leaseFilter !== 'all' || accessFilter !== 'all' || sort !== 'name';
  ...
  const handleBulkInvite = async () => {
    const selected = filteredTenants.filter(
      (tenant) => selectedTenantIds.has(getId(tenant)) && !hasAccount(tenant) && read(tenant, 'email', 'Email')
    );
  ...
  return (
    <Box sx={{ pb: 3 }}>
      ...
      >
        ...
        {isLoading ? (
          ...
          <Stack alignItems="center" spacing={1.4} sx={{ py: 7, px: 2, textAlign: 'center' }}>
            ...
            <Typography sx={{ color: 'text.secondary', fontSize: '0.85rem', maxWidth: 430 }}>
              Create a tenant record to organize contact details, lease placement, and portal access.
            </Typography>
            <Button
              variant="contained"
              color="success"
          ...
          <Stack alignItems="center" spacing={1.5} sx={{ py: 7, px: 2, textAlign: 'center' }}>
            ...
            <Typography sx={{ color: 'text.secondary', fontSize: '0.85rem' }}>
              Try a different search or reset the tenant filters.
            </Typography>
      ...
      >
        <MenuItem
          onClick={() => {
            setTenantToEdit(actionMenuTenant);
        ...
        {hasLease(actionMenuTenant) ? (
          <MenuItem
            onClick={() => {
              const leaseId = read(actionMenuTenant, 'leaseId', 'LeaseId');
              closeActionMenu();
        ...
        {actionMenuTenant && !hasAccount(actionMenuTenant) && read(actionMenuTenant, 'email', 'Email') && (
          <MenuItem
            disabled={sendingInvite[getId(actionMenuTenant)]}
            onClick={() => {
              const tenant = actionMenuTenant;
              closeActionMenu();
              handleSendInvite(tenant);
            }}
          >
            <Stack direction="row" spacing={1} alignItems="center">
              <MailOutlined />
              <span>{tenantInvites[getId(actionMenuTenant)] ? 'Resend invite' : 'Send invite'}</span>
            </Stack>
          </MenuItem>
        )}
        <MenuItem
          sx={{ color: 'error.main' }}
          onClick={() => {
            setTenantToRemove(actionMenuTenant);
      ...
      <AddToLeaseDrawer
        ...
        onClose={() => {
          setAddToLeaseDrawerOpen(false);
          setAddToLeaseTenant(null);
        }}
        onSuccess={refetch}
      />
    </Box>
