      ...
      <Stack spacing={2.25}>
        <Box>
          <Typography variant="h5" fontWeight={800}>
            Set up your dedicated SMS number
          </Typography>
            ...
            <CardContent>
              <Stack spacing={1.5}>
                ...
                <Typography variant="body2" color="text.secondary">
                  Upgrade to Premium to choose a Property Peace texting number for tenant communication. Tenants text normally, and replies
                  stay grouped in your app inbox.
                </Typography>
                <Button
                  variant="contained"
            ...
            <CardContent>
              <Stack spacing={2}>
                <Box>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    Pick a state, select an area code, then choose from live Twilio inventory. Your Premium plan includes one dedicated
                    number.
                  </Typography>
                </Box>
                ...
                <Button
                  ...
                  disabled={!state || !areaCode || searching}
                ...
                {numbers.length > 0 && (
                  <Stack spacing={1}>
                    <Typography variant="subtitle2" fontWeight={800}>
                      {selectedNumber ? 'Selected number' : 'Available numbers'}
                    </Typography>
                    {visibleNumbers.map((number) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <ListItemText
                            primary={formatPhone(number.phoneNumber)}
                            secondary={[number.locality, number.region].filter(Boolean).join(', ')}
                          />
        ...
        <Box>
          ...
          <Stack spacing={1.25}>
            {[
              [
                'Keep tenant messages separate',
                'Use a Property Peace number instead of mixing rental texts with your personal messages.',
                <MobileOutlined />
              ],
              ...
              [
                ...
                'Keep rent, lease, maintenance, and general conversations searchable in one place.',
                <SafetyOutlined />
              ],
              [
                'Look more professional',
                'Give tenants a dedicated rental communication number instead of your personal cell.',
                <CrownOutlined />
              ]
            ].map(([title, body, icon]) => (
              <Stack
                ...
                sx={{ p: 1.25, borderRadius: 2, bgcolor: alpha(theme.palette.text.primary, theme.palette.mode === 'dark' ? 0.06 : 0.035) }}
              >
  ...
  return (
    <>
      ...
      <ThemeAdaptiveDrawer
        ...
        open={open}
        onClose={closeDrawer}
        PaperProps={{ sx: { width: { xs: '100%', sm: 480 }, maxWidth: '100%', bgcolor: 'background.paper' } }}
      >
          ...
          {!selectedStep && (
            <Box
              sx={(t) => ({
                ...
                bgcolor: SETUP_NAVY,
                '& .MuiTypography-root': { color: t.palette.common.white }
              })}
            >
              <Stack direction="row" alignItems="flex-start" justifyContent="space-between" spacing={2}>
                <Box>
                  <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 1 }}>
                    ...
                    <Typography variant="h5" fontWeight={800} sx={{ color: 'common.white' }}>
                      Account setup
                    </Typography>
                  </Stack>
                  <Typography variant="body2" sx={{ color: alpha('#fff', 0.86), maxWidth: 340 }}>
                    Complete the essentials to get Property Peace ready for rent collection, tenant communication, and day-to-day
                    operations.
                  </Typography>
                </Box>
                <IconButton onClick={closeDrawer} sx={{ color: 'common.white', mt: -0.5 }} aria-label="Close setup drawer">
                  <CloseOutlined />
                </IconButton>
              </Stack>
              ...
              <Box sx={{ mt: 2.5 }}>
                <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 0.75 }}>
                  <Typography variant="caption" sx={{ color: SETUP_ACCENT_GREEN, fontWeight: 800 }}>
                ...
                <LinearProgress
                  ...
                  sx={{
                    height: 8,
                    borderRadius: 999,
                    bgcolor: alpha(SETUP_ACCENT_GREEN, 0.22),
                    '& .MuiLinearProgress-bar': { borderRadius: 999, bgcolor: SETUP_ACCENT_GREEN }
                  }}
                />
              </Box>
            </Box>
            ...
            {selectedStep?.kind === 'sms-number' ? (
              ...
              Object.entries(groupedSteps).map(([groupName, groupSteps], groupIndex) => (
                <Box key={groupName} sx={{ mb: 2.25 }}>
                  ...
                  <Typography variant="overline" color="text.secondary" fontWeight={800} sx={{ letterSpacing: 0.6 }}>
                    {groupName}
                  </Typography>
                    ...
                    {groupSteps.map((step) => (
                      <ListItemButton
                        ...
                        sx={{
                          ...
                          borderRadius: 2,
                          border: (t) =>
                            `1px solid ${step.completed ? alpha(SETUP_GREEN, t.palette.mode === 'dark' ? 0.32 : 0.18) : alpha(t.palette.divider, 0.78)}`,
                          bgcolor: (t) =>
                            step.completed ? alpha(SETUP_GREEN, t.palette.mode === 'dark' ? 0.12 : 0.07) : 'background.paper',
                          '&:hover': {
                            bgcolor: (t) =>
                              step.completed
                                ? alpha(SETUP_GREEN, t.palette.mode === 'dark' ? 0.16 : 0.1)
                      ...
                      >
                        <ListItemIcon sx={{ minWidth: 36 }}>
                          {step.completed ? (
                            <CheckCircleFilled style={{ color: SETUP_GREEN, fontSize: 21 }} />
                          ) : (
                            <Box
                              sx={(t) => ({
                                width: 21,
                                height: 21,
                                borderRadius: '50%',
                                border: `2px solid ${alpha(t.palette.text.secondary, 0.32)}`
                              })}
                            />
                          )}
                        </ListItemIcon>
                            ...
                            <Stack direction="row" alignItems="center" spacing={1}>
                              ...
                              {step.badge && (
                                <Chip
                                  label={step.badge}
                                  size="small"
                                  color={step.badge === 'Premium' ? 'primary' : step.badge === 'Active' ? 'success' : 'default'}
                                  variant="outlined"
                        ...
                        />
                        {step.completed ? (
                          <CheckOutlined style={{ color: SETUP_GREEN, fontSize: 16 }} />
                        ) : (
                          <RightOutlined style={{ color: theme.palette.text.secondary, fontSize: 14 }} />
                        )}
                      </ListItemButton>
                    ))}
                  </List>
