...
export default function EcommerceMetrix({ primary, secondary, content, iconPrimary, color, sx, ...props }) {
    ...
    >
      <Box sx={(theme) => ({ px: 4.5, py: 4, color: 'background.paper', ...theme.applyStyles('dark', { color: 'text.primary' }) })}>
        <Grid container justifyContent="space-between" alignItems="center">
          <Grid>
            <Typography sx={(theme) => ({ opacity: 0.23, fontSize: 46, lineHeight: 0, ...theme.applyStyles('dark', { opacity: 0.5 }) })}>
              {primaryIcon}
            </Typography>
          </Grid>
          <Grid>
