...
import { getMedianDOMReport } from 'api/reports';

import useAuth from 'hooks/useAuth';
import { ResponsiveContainer, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend } from 'recharts';

export default function MedianDOMReport() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const data = await getMedianDOMReport({
  ...
  return (
    <Box>
      <PageBreadcrumbs items={[
        ...
        { label: 'Median DOM Report' }
      ]} />
      <MainCard>
        <Stack spacing={3}>
          <Box>
            <Typography variant="h4" fontWeight="bold" gutterBottom>Median DOM Report</Typography>
            <Typography variant="body2" color="text.secondary">Time it takes to lease a unit</Typography>
            <Typography variant="caption" color="text.secondary" sx={{ fontStyle: 'italic', display: 'block', mt: 0.5 }}>
              Formula: DOM = Days on Market
            </Typography>
          </Box>
          ...
          {!loading && !error && reportData && (
            <Grid container spacing={3}>
              <Grid size={{ xs: 12, md: 4 }}>
                <Card><CardContent>
                  <Typography variant="subtitle2" color="text.secondary" gutterBottom>Median Days on Market</Typography>
                  <Typography variant="h3" fontWeight="bold" color="primary">{reportData.medianDOM || 0} days</Typography>
                </CardContent></Card>
              </Grid>
              <Grid size={{ xs: 12, md: 4 }}>
                <Card><CardContent>
                  <Typography variant="subtitle2" color="text.secondary" gutterBottom>Average DOM</Typography>
                  <Typography variant="h3" fontWeight="bold">{reportData.averageDOM?.toFixed(1) || 0} days</Typography>
                </CardContent></Card>
              </Grid>
              <Grid size={{ xs: 12, md: 4 }}>
                <Card><CardContent>
                  <Typography variant="subtitle2" color="text.secondary" gutterBottom>Total Units Leased</Typography>
                  <Typography variant="h3" fontWeight="bold">{reportData.totalUnitsLeased || 0}</Typography>
                </CardContent></Card>
              </Grid>
              {reportData.domByProperty && reportData.domByProperty.length > 0 && (
                <Grid size={{ xs: 12 }}>
                  <MainCard title="DOM by Property">
                    <ResponsiveContainer width="100%" height={300}>
                      <BarChart data={reportData.domByProperty}>
                        <CartesianGrid strokeDasharray="3 3" />
                        <XAxis dataKey="propertyName" />
                        <YAxis />
                        <Tooltip formatter={(value) => `${value} days`} />
                        <Legend />
                        <Bar dataKey="dom" fill="#eb2f96" name="Days on Market" />
                      </BarChart>
                    </ResponsiveContainer>
                  </MainCard>
                </Grid>
