...
// material-ui
import { Grid, Link, Stack, Fade, Tooltip, IconButton, Collapse } from '@mui/material';
...
import DashboardHeader, { DashboardReminderCard } from 'sections/landlord/dashboard/DashboardHeader';
...
import { buildTodayReminders } from 'utils/taskWorkspace';
import { readRecentlyViewedPropertyIds, resolveRecentlyViewedProperties } from 'utils/recentlyViewedProperties';
...
export default function DashboardOverview({ embedded = false }) {
  ...
  const { expenses, refetch: refetchExpenses, loading: expensesLoading } = useFetchExpenses(expenseFilters);
  ...
  const recentlyViewedProperties = useMemo(
    () => resolveRecentlyViewedProperties(readRecentlyViewedPropertyIds(user), properties),
    [user, properties]
  );
  ...
  const isTrialExpired =
    subscriptionStatus?.isTrialActive && (subscriptionStatus?.trialDaysRemaining === null || subscriptionStatus?.trialDaysRemaining <= 0);

  // Get user ID (handle both Id and id for compatibility)
  const userId = user?.Id || user?.id;

  const setupDrawerRequested = searchParams.get('setup') === 'open';
  const [setupTasksOpen, setSetupTasksOpen] = useState(setupDrawerRequested);
  const [isDismissingSetup, setIsDismissingSetup] = useState(false);
  const closeSetupTasks = () => {
    setSetupTasksOpen(false);
    if (setupDrawerRequested) {
  ...
  const dismissSetupCard = async () => {
    ...
    setIsDismissingSetup(true);
    try {
      await axiosServices.put('/api/user/tutorial-status', { hasSeenTutorial: true });
      updateUser?.({ HasSeenTutorial: true, hasSeenTutorial: true });
      closeSetupTasks();
    } catch (error) {
      openSnackbar({
        open: true,
        message: error?.response?.data?.message || 'We could not dismiss account setup. Please try again.',
        variant: 'alert',
        alert: { color: 'error' }
      });
    } finally {
      setIsDismissingSetup(false);
    }
  };
  const setupState = useLandlordSetupSteps(closeSetupTasks);

  useEffect(() => {
    if (setupDrawerRequested) setSetupTasksOpen(true);
  }, [setupDrawerRequested]);
  ...
  const showSetupCard = !(user?.HasSeenTutorial || user?.hasSeenTutorial) && (setupState.isLoading || !setupState.isComplete);
  ...
  return (
    <>
      <Fade in={fadeIn} timeout={600}>
        ...
        >
          <Grid container spacing={2.5} sx={isTrialExpired ? { filter: 'blur(4px)', pointerEvents: 'none', userSelect: 'none' } : {}}>
            {!embedded && (
              <Grid size={12}>
                <AnimateIn direction="bottom" delay={100} distance={120}>
                  <Box sx={{ mt: { xs: 2, sm: 2, md: 0 } }}>
                    <DashboardHeader userName={user?.firstname || user?.Firstname} />
                  </Box>
                </AnimateIn>
              </Grid>
            )}
