...
import {
  Alert, Box, Button, Chip, CircularProgress, Divider, Grid, Paper, Stack, TextField, Typography, alpha
} from '@mui/material';
...
import {
  availableMaintenanceActions, createEvidenceUploadEntries, evidenceSelection, maintenanceActorForRoute, maintenanceStatusSelectionCommand, maintenanceUserId,
  MAINTENANCE_EVIDENCE_TYPES, statusLabel, uploadPendingEvidence, workflowActivitiesFromMaintenanceDetail,
  workflowFromMaintenanceDetail
...
export default function MaintenancePage() {
  ...
  const hasDetailedWorkflowAction = [
    'submitEstimate', 'decideEstimate', 'issueWorkOrder', 'schedule', 'cancelWorkOrder', 'cancelAppointment', 'start', 'complete', 'staffClose'
  ].some((key) => actions[key]);
  ...
  return <Box sx={{ pb: 5 }}>
    ...
    <Box sx={{ mt: 2, mb: 3 }}>
      ...
      <Stack direction="row" spacing={1.25} alignItems="center" flexWrap="wrap" useFlexGap sx={{ mt: 1.25 }}>
        ...
        <Chip label={statusLabel(request.status)} variant="outlined" />
      </Stack>
    </Box>
    ...
    <Grid container spacing={2} sx={{ mt: .25 }}>
      <Grid size={{ xs: 12, lg: role === 'manager' ? 8 : 12 }} sx={{ order: { xs: 2, lg: 1 } }}><Stack spacing={2}>
        ...
        <Section eyebrow="WORKFLOW" title="Explicit maintenance actions">
          ...
          {!hasDetailedWorkflowAction && <Alert severity="info">{role === 'manager' ? 'Use the Actions panel to move this request forward.' : 'No workflow action is valid for your role and the current state.'}</Alert>}
