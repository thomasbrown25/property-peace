...
export default function useLandlordSetupSteps(onClose) {
  ...
  const allUnits = useMemo(
    () => activeProperties.flatMap((property) => property.units || property.Units || []),
    [activeProperties]
  );
  ...
  const activeLeases = useMemo(
    () =>
      allUnits
        .map((unit) => unit.lease || unit.Lease)
        .filter((lease) => lease && (lease.isActive ?? lease.IsActive ?? true)),
    [allUnits]
  );
  ...
  const hasPremiumOrLifetimeSubscription = useMemo(() => {
    ...
    return planName === 'premium' || planName.includes('lifetime') || billingCycle === 'lifetime' || primarySubscription?.cancelAtPeriodEnd === true || primarySubscription?.CancelAtPeriodEnd === true;
  ...
  const steps = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        group: 'Portfolio',
        title: 'Add units',
        description: firstPropertyIsSingleFamily ? 'Single-family properties are ready to manage without extra units.' : 'Set up the unit or units tenants will occupy.',
      ...
      },
      {
        ...
        badge: effectiveSmsNumberStatus?.hasActiveNumber ? 'Active' : effectiveSmsNumberStatus?.hasPremiumAccess ? 'Recommended' : 'Premium',
