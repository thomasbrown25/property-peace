...
export const PASSWORD_REQUIREMENTS_TEXT = `Password must be ${PASSWORD_MIN_LENGTH}-${PASSWORD_MAX_LENGTH} characters and include at least one uppercase letter, one lowercase letter, one number, and one special character (${PASSWORD_SPECIAL_CHARACTERS}).`;
...
export function validatePassword(password) {
  ...
  if (!PASSWORD_SPECIAL_REGEX.test(password)) {
    return `Password must contain at least one special character (${PASSWORD_SPECIAL_CHARACTERS})`;
  }
