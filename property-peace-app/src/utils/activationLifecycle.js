...
function routeDescriptor(key, context, mode = 'vacant') {
  ...
  const routes = {
    ...
    'tenant-invite': [leaseId ? `/landlord/leases/${leaseId}/add-tenant${tenantId ? `?tenantId=${tenantId}` : ''}` : tenantId ? `/landlord/tenants/${tenantId}` : '/landlord/leases', leaseId ? 'Add or invite tenant' : tenantId ? 'Open tenant' : 'Choose a lease'],
