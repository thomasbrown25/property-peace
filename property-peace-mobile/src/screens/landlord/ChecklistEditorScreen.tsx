...
export default function ChecklistEditorScreen({ route }: Props) {
  ...
  const savePair = async (nextActive: Checklist, nextCounterpart: Checklist | null) => {
    await saveActive(nextActive);
    if (nextCounterpart?.id != null) {
      try {
        setCounterpart(await ChecklistAPI.update(nextCounterpart.id, nextCounterpart));
      } catch (pairError) {
        await load();
        throw new Error(`The active checklist changed, but the connected checklist did not. Retry the room change. ${errorMessage(pairError, '')}`.trim());
      }
    }
  };
  ...
  const changeCondition = async (item: ChecklistItem, requested: ChecklistCondition | null) => {
    ...
    setChecklist(next);
    setSavingKey(`condition-${idText(item.id)}`);
  ...
  const addItem = async (room: ChecklistRoom) => {
    if (!checklist) return;
    setSavingKey(`add-item-${room.name}`);
    try {
  ...
  const confirmDelete = (item: ChecklistItem) => {
    ...
    Alert.alert('Delete checklist item?', `Remove “${item.name}” from this inspection?`, [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => {
        setSavingKey(`delete-${idText(item.id)}`);
        try {
        ...
        {!collapsed && (
          <View>
            <View style={styles.renameRow}>
              <TextInput value={renameValue} onChangeText={(value) => setRenameDrafts((current) => ({ ...current, [room.name]: value }))} style={styles.smallInput} accessibilityLabel={`Rename ${room.name}`} />
              <TouchableOpacity style={styles.secondaryButton} onPress={() => renameRoom(room)} disabled={savingKey === `rename-${room.name}`}>
            ...
            <View style={styles.addItemRow}>
              ...
              />
              <TouchableOpacity style={styles.addButton} onPress={() => addItem(room)} disabled={!itemNameDrafts[room.name]?.trim() || savingKey === `add-item-${room.name}`}>
        ...
        <View>
          ...
          <View style={styles.addRoomCard}>
            ...
            <View style={styles.addRoomRow}>
              <TextInput value={roomDraft} onChangeText={setRoomDraft} placeholder="Room name" placeholderTextColor="#8795a1" style={styles.smallInput} />
              <TouchableOpacity style={styles.addButton} onPress={addRoom} disabled={!roomDraft.trim() || savingKey === 'add-room'}>
  ...
  return (
    <View style={styles.itemCard}>
      <View style={styles.itemTop}>
        ...
        {(item.sortOrder ?? 0) >= 1000 && <TouchableOpacity style={styles.iconButton} onPress={onDelete} disabled={savingKey === `delete-${key}`}><Ionicons name="trash-outline" size={19} color="#c2413b" /></TouchableOpacity>}
      </View>
      <View style={styles.conditions}>
        {CHECKLIST_CONDITIONS.map((option) => {
          const selected = item.condition === option.value;
          return <TouchableOpacity key={option.value} style={[styles.condition, selected && styles.conditionSelected]} onPress={() => onCondition(option.value)} disabled={savingCondition}><Text style={[styles.conditionText, selected && styles.conditionTextSelected]}>{option.value}</Text></TouchableOpacity>;
        })}
      </View>
      {savingCondition && <Text style={styles.savingText}>Saving condition…</Text>}
      {!!retry && <TouchableOpacity onPress={() => onCondition(retry.condition)}><Text style={styles.retryInline}>Condition not saved · Retry</Text></TouchableOpacity>}
      <TextInput
        value={draft.notes}
...
const styles = StyleSheet.create({
  ...
  condition: { minWidth: 44, minHeight: 38, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: '#d7e0e6', borderRadius: 11, backgroundColor: '#f8fafb', paddingHorizontal: 8 },
  ...
  saveDetailsButton: { alignSelf: 'flex-start', minHeight: 40, justifyContent: 'center', paddingHorizontal: 13, borderRadius: 11, backgroundColor: '#eaf3ff', marginTop: 8 },
