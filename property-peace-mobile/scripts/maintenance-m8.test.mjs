...
test('tenant role detection accepts the real camelCase auth payload',()=>{assert.deepEqual(extractRoles({roles:['Tenant']}),['tenant']);assert.deepEqual(extractRoles({currentOrganizationRole:'Tenant'}),['tenant']);assert.equal(maintenanceAudience({currentOrganizationRole:'Tenant'}),'tenant');assert.match(nav,/TenantMaintenanceNavigator/);const tabs=nav.slice(nav.lastIndexOf("{audience === 'tenant' ? <>"),nav.lastIndexOf('</Tab.Navigator>'));const tenantBranch=tabs.slice(0,tabs.indexOf("</> : audience === 'landlord'"));assert.match(tenantBranch,/name="Maintenance"/);assert.match(tenantBranch,/name="Messages"/);assert.match(tenantBranch,/name="Settings"/);assert.doesNotMatch(tenantBranch,/name="Dashboard"|name="Properties"|name="Tenants"/);});
...
test('role routing is active-context aware and never sends vendors to landlord screens',()=>{
 assert.match(model,/maintenanceAudience/);assert.equal(extractRoles({roles:['Landlord','Tenant'],currentOrganizationRole:'Tenant'})[0],'tenant');assert.match(nav,/audience === 'tenant'/);assert.match(nav,/audience === 'landlord'/);assert.match(nav,/UnsupportedMaintenanceNavigator/);assert.doesNotMatch(nav,/const tenant = isTenantUser/);
});
